...
public interface PesquisaDAO {
    ...
    @Query("SELECT est_voto, COUNT(*) as count FROM tbEstimulada group BY est_voto order by count desc")
    ...
    @Query("SELECT prob_problemaId, COUNT(*) as contagem from tbproblema group by prob_problemaId order by contagem desc")
